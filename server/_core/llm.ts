...
const resolveApiUrl = () => {
  const baseUrl = ENV.forgeApiUrl && ENV.forgeApiUrl.trim().length > 0 ? ENV.forgeApiUrl.trim().replace(/\/$/, "") : "https://api.openai.com/v1";
...
export async function invokeLLM(params: InvokeParams): Promise<InvokeResult> {
  ...
  if (model) {
    payload.model = model === "auto" ? ENV.defaultLlmModel : model;
  } else {
    payload.model = ENV.defaultLlmModel;
  }
  ...
  console.log("[LLM] Invoking API:", apiUrl);
